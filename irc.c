	...
	// Parses IRC message that pulls out nick and message. 
		...
		*irc_msg = '\0';
		...
		if (irc->servbuf[0] == ':') {
			...
			if (privmsg == 1 && strlen(irc_nick) > 0
			    && strlen(irc_msg) > 0) {
				irc_log_message(irc, irc_nick, irc_msg);
				if (irc_reply_message(irc, irc_nick, irc_msg) <
				    0)
					return -1;
			}
		}
...
irc_reply_message(irc_t * irc, char *irc_nick, char *msg)
{
	...
	if (strcmp(command, "ping") == 0) {
		...
		if (arg != NULL && strlen(arg) > 0) {
			if (critical)
				snprintf(mesg, 511,
					 "I smack thee, %s, for %d damage (it's super effective).",
					 arg, rand() % 20 + 21);

			else
				snprintf(mesg, 511,
					 "I smack thee, %s, for %d damage.",
					 arg, rand() % 20 + 1);
			mesg[511] = '\0';
		}
			...
			snprintf(mesg, 511,
				 "Behold, I smack thee, %s, for %d damage.",
				 irc_nick, rand() % 20 + 1);
			mesg[511] = '\0';
		}
		...
		snprintf(mesg, 511, "%s: http://lmgtfy.com/?q=%s",
			 t_nick, link);
		mesg[511] = '\0';
		if (irc_msg(irc->s, irc->channel, mesg) < 0)
			return -1;
